    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      >
        ...
        <div className="h-16 flex items-center px-4 border-b border-sidebar-border gap-3">
          <div className="w-8 h-8 rounded-lg gradient-luxury flex items-center justify-center flex-shrink-0">
            <Store className="w-4 h-4 text-primary-foreground" />
          </div>
          {sidebarOpen && (
            <div className="overflow-hidden">
              <p className="font-luxury text-sm font-semibold text-sidebar-foreground leading-tight">Hayaat</p>
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          {navItems.map(({ to, label, icon: Icon, end }) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon className={cn('w-5 h-5 flex-shrink-0', isActive ? 'text-primary-foreground' : 'text-muted-foreground group-hover:text-sidebar-accent-foreground')} />
          ...
          {sidebarOpen && (
            <div className="px-3 py-2 rounded-lg bg-sidebar-accent">
              <p className="text-xs font-medium text-sidebar-foreground truncate">
                {profile?.display_name || profile?.first_name || 'Admin'}
              </p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 flex items-center px-6 border-b border-border bg-card flex-shrink-0 gap-4">
          ...
          <div className="text-sm text-muted-foreground">
            Welcome back, <span className="text-foreground font-medium">{profile?.first_name || 'Admin'}</span>
          </div>
        </header>
