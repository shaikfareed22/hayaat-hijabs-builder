...
import { Plus, Pencil, Trash2, Search, Package, Images } from 'lucide-react';
import { toast } from '@/hooks/use-toast';
import { Textarea } from '@/components/ui/textarea';
import { ProductImageManager } from '@/components/admin/ProductImageManager';

type VariantWithImages = Pick<Tables<'product_variants'>, 'id' | 'price' | 'stock_quantity' | 'color' | 'size' | 'sku'> & {
...
export default function AdminProducts() {
  ...
  const fetchProducts = async () => {
    ...
    const { data, error } = await supabase
      .from('products')
      .select('*, product_variants(id, price, stock_quantity, color, size, sku, product_images(id, image_url, is_primary, display_order, variant_id, alt_text))')
  ...
  return (
    <div className="p-6 space-y-6">
                ...
                return (
                  <div key={product.id} className="flex items-center gap-4 px-6 py-4 hover:bg-muted/30 transition-colors">
                    <div className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center overflow-hidden flex-shrink-0">
                      {productPrimaryImage ? (
                        <img
                          src={productPrimaryImage.image_url}
                          alt={productPrimaryImage.alt_text || `${product.name} image`}
                    ...
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" onClick={() => setImageProduct({ id: product.id, name: product.name })}>
      ...
      {imageProduct && (
        <ProductImageManager
          open={!!imageProduct}
          onOpenChange={(next) => !next && setImageProduct(null)}
