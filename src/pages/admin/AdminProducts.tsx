...
type Product = Tables<'products'> & {
  product_variants: Pick<Tables<'product_variants'>, 'id' | 'price' | 'stock_quantity' | 'color' | 'size' | 'sku'>[];
};
...
export default function AdminProducts() {
  ...
  const fetchProducts = async () => {
    ...
    const { data, error } = await supabase
      .from('products')
      .select('*, product_variants(id, price, stock_quantity, color, size, sku)')
    ...
    <div className="p-6 space-y-6">
                ...
                return (
                  <div key={product.id} className="flex items-center gap-4 px-6 py-4 hover:bg-muted/30 transition-colors">
                    ...
                    <div className="hidden md:flex items-center gap-6 text-sm">
                      ...
                      <div className="text-center">
                        <p className={`font-medium ${totalStock < 5 ? 'text-destructive' : ''}`}>{totalStock}</p>
                        <p className="text-xs text-muted-foreground">Stock</p>
                      </div>
                    </div>
                    <Badge variant={product.is_active ? 'default' : 'secondary'} className="text-xs hidden sm:flex">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-luxury">{editing ? 'Edit Product' : 'Add Product'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Name *</Label>
                <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') }))} placeholder="Elegant Hijab" />
              </div>
              <div>
                <Label>Slug *</Label>
                <Input value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} placeholder="elegant-hijab" />
              </div>
            </div>
            <div>
              <Label>Short Description</Label>
              <Input value={form.short_description} onChange={e => setForm(f => ({ ...f, short_description: e.target.value }))} placeholder="Brief product description" />
            </div>
            <div>
              <Label>Description</Label>
              <Textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Full product description..." rows={3} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Fabric Type</Label>
                <Input value={form.fabric_type} onChange={e => setForm(f => ({ ...f, fabric_type: e.target.value }))} placeholder="Chiffon, Silk..." />
              </div>
              <div>
                <Label>Care Instructions</Label>
                <Input value={form.care_instructions} onChange={e => setForm(f => ({ ...f, care_instructions: e.target.value }))} placeholder="Hand wash cold" />
              </div>
            </div>
