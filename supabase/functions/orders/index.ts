    ...
    switch (req.method) {
      case 'POST': {
        ...
        if (coupon_id) {
          ...
          await supabaseAdmin.from('coupons').update({ used_count: (await supabaseAdmin.from('coupons').select('used_count').eq('id', coupon_id).single()).data?.used_count + 1 }).eq('id', coupon_id)
        }
